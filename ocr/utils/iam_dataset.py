...
class IAMDataset(dataset.ArrayDataset):
    ...
    def __init__(self, parse_method, credentials=None,
        ...
        if self._output_data == "bb":
            ...
            self.image_data_file_name = os.path.join(root, "image_data-{}-{}-{}*.sav".format(
                self._parse_method, self._output_data, self._output_parse_method))
        else:
            self.image_data_file_name = os.path.join(root, "image_data-{}-{}*.sav".format(self._parse_method, self._output_data))
    ...
    def _save_dataframe_chunks(self, df, name):
        for i, df_split in enumerate(np.array_split(df, 70)):
            print("len "+str(len(df_split)))
            filename = name[:-5] + str(i) + ".sav" # remove *.plk in the filename
            joblib.dump(df_split,filename)
    ...
    def _process_data(self):
        ...
        #d1,d2,d3,d4,d5,d6,d7,d8,d9,d10=np.array_split(image_data, 10)
